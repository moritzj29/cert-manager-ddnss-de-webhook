...
func TestLoadConfig_InvalidConfigName(t *testing.T) {
	...
	assert.Contains(t, err.Error(), "no api token secret name provided in ddnss.de config", "Expected specific error message for missing API token secret")
}
...
func TestLoadConfig_InvalidConfiKey(t *testing.T) {
	...
	_, err := LoadConfig(cfgJSON)
	assert.Error(t, err, "Expected error for invalid config")
	assert.Contains(t, err.Error(), "no api token secret key provided in ddnss.de config", "Expected specific error message for missing API token secret")
}
